...
export default function PpmpMasterlist() {
    ...
    const loadPpmpTableData = async () => {
      ...
      try {
        ...
        if (!masterlistResponse.ok) {
          toast.error(
            "Failed to fetch PPMP masterlist data. Please try again later.",
          );
        } else {
  ...
  const ItemsCountCardData: {
    ...
    {
      icon: "package",
      title: "Total Items in Planned",
      count: totalPlannedItemCount,
      color: "gray",
    },
    {
      icon: "chart",
      title: "Total Available Items",
      count: totalAvailableItemCount,
      color: "blue",
    },
    {
      icon: "clock",
    ...
    {
      icon: "check",
      title: "Total Fulfilled Items",
      count: totalFulfilledItemCount,
      color: "green",
    },
    {
      icon: "businessplan",
      title: "Total Planned Price",
      count: totalPlannedFunds,
      color: "royal-red",
    },
  ];

  const exportLatestPPMP = async (options: string[]) => {
    const formData = new FormData();
    formData.append("year", String(selectedFiscalYear));
    ...
    try {
      ...
      const response = await fetch(
        "https://test-ppmp.onrender.com/api/export/",
        {
          method: "POST",
          body: formData,
          headers: {
      ...
      if (!response.ok) {
        toast.error(
          "Failed to export the latest PPMP. Please try again later.",
        );
      } else {
  ...
  return (
    <main className="page-container masterlist">
      <LoadingWrapper
        ...
        skeleton={<PpmpMasterlistSkeleton />}
      >
