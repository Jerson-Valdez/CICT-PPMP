...
export default function ViewInLieu({
  ...
  const { selectedFiscalYear, userRole, deanName, revisedAsignatories } =
    useOutletContext<{
  ...
  const requestMonthIndex = parsedRequestDate
  ...
  return (
    <dialog className="view-in-lieu" ref={dialogRef} onCancel={handleCancel}>
      ...
      <div className="content" ref={printRef}>
        <div className="title">
          <h3>
            REVISED PROJECT PROCUREMENT MANAGEMENT PLAN {selectedFiscalYear}
          </h3>
        </div>
        <p>
          END-USER/UNIT: <u>CICT</u>
        </p>
              ...
              originalItems.map((item, index) => (
                <tr key={item.itemId}>
                  ...
                  <td className="text-right">
                    {(item.priceCatalog * item.quantity).toFixed(2)}
                  </td>
                </tr>
              ))}
            <tr>
              <td colSpan={2} className="text-right bg-gray-200">
                <strong>TOTAL AMOUNT</strong>
              </td>
              <td colSpan={16} className="text-right bg-gray-200">
                <strong>
                  {originalItems
                    ? originalItems
                      .reduce(
                        (total, item) =>
                          total + item.priceCatalog * item.quantity,
                        0,
                      )
        ...
        <div className="signatory">
          ...
          <div className="noted-by">
            <p>Noted By:</p>
            <div>
              {revisedAsignatories && revisedAsignatories.length > 0 &&
                revisedAsignatories.map((signatory: any, index: number) => (
                  <div key={index}>
                    <p className="signatory-name">
