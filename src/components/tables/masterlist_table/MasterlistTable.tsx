...
import ExportPromptDialog from "../../dialogs/export_prompt_dialog/ExportPromptDialog";
...
export default function MasterlistTable({
  ...
  let processedData = data.filter((item) => {
    ...
    const matchesSearch =
      searchTerm === "" || item.itemName.toLowerCase().includes(searchLower);

    let matchesStatus = true;
    if (statusFilter === "pending") matchesStatus = item.pendingQuantity > 0;
    if (statusFilter === "fulfilled")
      matchesStatus = item.fulfilledQuantity > 0;
    if (statusFilter === "available")
      matchesStatus = item.availableQuantity > 0;

    const matchesItemCat =
      itemCatFilter === "" || item.itemCategory === itemCatFilter;
    const matchesPpmpCat =
      ppmpCatFilter === "" || item.ppmpCategory === ppmpCatFilter;
    ...
    <div className="table-container masterlist">
      <div className="table-title-container">
        <div className="table-title">
          ...
          <p>
            <span>{itemCount}</span> Items • <span>{unitCount}</span> Units
        ...
        >
          <IconFileTypeXls size={24} /> Export Datas
        </button>
        {isExportDialogOpen && (
          <ExportPromptDialog
          ...
          <tbody>
            {processedData.map((item, index) => (
              <tr key={index}>
                ...
                <td>
                  {item.priceCatalog.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
                </td>
                <td>
                  {(item.plannedQuantity * item.priceCatalog).toLocaleString(
                    undefined,
                ...
                <td>
                  {item.availableQuantity > 0 ? (
                    <>
                      ...
                      >
                        <IconFileStack size={18} /> Create PR (
